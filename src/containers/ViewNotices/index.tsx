...
export default function ViewNotices({ tempState,settempState }: { tempState: number,settempState:React.Dispatch<React.SetStateAction<number>> }) {
  ...
  useEffect(() => {
    if (Notices.length >= 1) { 
      let doc = document.getElementById('notice_delete_btn') as HTMLDivElement | null
  ...
  return (
    <div className="ViewNotices">
      ...
      <Row>
        {Notices!= undefined && Notices.length >= 1 && (
